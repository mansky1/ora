...
Local Definition lc_supply_def `{!lcGS Σ} (n : nat) : iProp Σ := own(A := nat_authUR) lcGS_name (● n).
...
Section later_credit_theory.
  ...
  Proof.
    ...
    iDestruct "H" as (?%nat_included) "_"; done.
  Qed.
...
(* should we use incl_order for nat?
  Lemma lc_weaken {n} m :
    m ≤ n → £ n -∗ £ m.
  Proof.
  ...
  Proof.
    rewrite lc_unseal /lc_def. apply own_core_persistent, @uora_unit_core_id.
  Qed.
...
Module le_upd.
  ...
  Definition le_upd_pre `{!lcGS Σ}
    ...
    (∀ n, lc_supply n ==∗
          (lc_supply n ∗ P) ∨ (∃ m, <affine> ⌜m < n⌝ ∗ lc_supply m ∗ ▷ le_upd P))%I.
  ...
  Local Lemma le_upd_unfold `{!lcGS Σ} P:
    (|==£> P) ⊣⊢
    ∀ n, lc_supply n ==∗
         (lc_supply n ∗ P) ∨ (∃ m, <affine> ⌜m < n⌝ ∗ lc_supply m ∗ ▷ le_upd P).
  Proof.
  ...
  Section le_upd.
    ...
    Lemma le_upd_elim n P :
      lc_supply n -∗
      (|==£> P) -∗
      Nat.iter n (λ P, |==> ▷ P) (|==> ◇ (∃ m, <affine> ⌜m ≤ n⌝ ∗ lc_supply m ∗ P)).
    Proof.
    ...
    Lemma le_upd_elim_complete n P `{!Absorbing P}:
      lc_supply n -∗
      (|==£> P) -∗
  ...
  Proof.
    ...
    iMod (own_alloc(A := nat_authUR) (● n ⋅ ◯ n)) as (γLC) "[H● H◯]";
  ...
  Proof.
    ...
    simpl. clear H. iInduction m as [|m] "IH"; simpl; [done|].
    iMod "H". iModIntro. iNext. by iApply "IH".
  Qed.
End le_upd.
