(* modified from iris.base_logic.lib.cancelable_invariants *)

From iris.algebra Require Export frac.
From iris.bi.lib Require Import fractional.
From iris.proofmode Require Import proofmode.
From iris_ora.algebra Require Import ext_order.
From iris_ora.logic Require Export invariants.
...
Section defs.
  ...
  Definition cinv_own (γ : gname) (p : frac) : iProp Σ := own(A := fracR) γ p.
...
Section proofs.
  ...
  Global Instance cinv_affine N γ P : Affine (cinv N γ P).
  Proof. rewrite /cinv; apply _. Qed.
  ...
  Proof. by iIntros "A B"; iPoseProof (own_valid_2 with "A B") as "%". Qed.
  ...
  Lemma cinv_alloc_cofinite (G : gset gname) E N :
    ⊢ |={E}=> ∃ γ, <affine> ⌜ γ ∉ G ⌝ ∗ cinv_own γ 1 ∗ ∀ P, ▷ P ={E}=∗ cinv N γ P.
  Proof.
