...
logic. This is done by solving the following recursive domain equation:
  ...
  iProp ≈ ouPred (∀ i : gid, gname -fin-> (Σ i) iProp)
...
category of ORAs with a proof that it is locally contractive. *)
Structure gFunctor := GFunctor {
  gFunctor_F :> OrarFunctor;
  gFunctor_map_contractive : OrarFunctorContractive gFunctor_F;
}.
...
Definition iResF (Σ : gFunctors) : uorarFunctor :=
...
Module Type iProp_solution_sig.
  ...
  Definition iResUR (Σ : gFunctors) : uora :=
    discrete_funUR (λ i,
      gmapUR gname (OrarFunctor_apply (gFunctors_lookup Σ i) (iPrePropO Σ))).
  Notation iProp Σ := (ouPred (iResUR Σ)).
  Notation iPropO Σ := (ouPredO (iResUR Σ)).
  Notation iPropI Σ := (ouPredI (iResUR Σ)).
...
Module Export iProp_solution : iProp_solution_sig.
  ...
  Definition iProp_result (Σ : gFunctors) :
    solution (ouPredOF (iResF Σ)) := solver.result _.
